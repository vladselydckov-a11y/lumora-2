...
export default function Page() {
  ...
  useEffect(() => {
    if (!authInfo || initialRouteApplied) return;

    // Владелец платформы сначала попадает в свою панель КЛИК,
    // а не сразу в ресторанный дашборд. Клиентские роли не трогаем.
    if (isPlatformOwnerUser(authInfo)) {
      setTab('platform');

...
function normalizeRequestedDateForPeriod(date, period) {
  const normalized = normalizePeriod(period);
  const selectedDate = String(date || getDefaultOperationalDate()).slice(0, 10);

  // Month selector in Telegram UI may represent June as 2026-06-01 -> 2026-07-01.
  // For analytics we need a closed business period: 2026-06-01 -> 2026-06-30.
  if (normalized === 'month' && /^\d{4}-\d{2}-01$/.test(selectedDate)) {
    return dateMinusDays(selectedDate, 1);
  }
...
function getRange({ date, period }) {
  const normalized = normalizePeriod(period);
  const endDate = normalizeRequestedDateForPeriod(date, normalized);
...
function isVisibleMenuItem(row) {
  ...
  if (badCategoryParts.some((part) => category.includes(part))) return false;

  const badExactNames = new Set(['доставка', 'delivery', 'самовывоз', 'pickup']);
  if (badExactNames.has(name)) return false;
...
function buildMenuCoverage(categories, revenueBase) {
  const detailRevenue = (Array.isArray(categories) ? categories : []).reduce((total, item) => total + toNumber(item?.revenue), 0);
  const totalRevenue = toNumber(revenueBase);
  const coveragePercent = totalRevenue > 0 ? Math.min(Math.round((detailRevenue / totalRevenue) * 1000) / 10, 100) : (detailRevenue > 0 ? 100 : 0);
  const unallocatedRevenue = Math.max(totalRevenue - detailRevenue, 0);
  const status = coveragePercent >= 85 ? 'good' : coveragePercent >= 50 ? 'warn' : 'bad';

  return {
    detailRevenue,
    detailRevenueText: formatMoney(detailRevenue),
    totalRevenue,
    totalRevenueText: formatMoney(totalRevenue),
    unallocatedRevenue,
    unallocatedRevenueText: formatMoney(unallocatedRevenue),
    coveragePercent,
    coverageText: `${coveragePercent}% меню распределено по блюдам/категориям`,
    status,
    note: status === 'good'
      ? 'Категории покрывают почти всю выручку периода.'
      : 'Часть выручки есть в KPI iiko, но пока не пришла в dish_sales. КЛИК показывает её отдельно, а не раскидывает по категориям фейково.'
  };
}

function categoriesWithUnallocated(categories, coverage) {
  const list = Array.isArray(categories) ? [...categories] : [];
  const missing = toNumber(coverage?.unallocatedRevenue);

  if (missing > Math.max(toNumber(coverage?.totalRevenue) * 0.01, 1)) {
    list.push({
      name: 'Не распределено по категориям',
      revenue: missing,
      revenueText: formatMoney(missing),
      cost: 0,
      costText: '0 ₽',
      quantity: 0,
      foodcost: 0,
      margin: 0,
      foodcostText: 'нет детализации',
      marginText: 'нет детализации',
      isUnallocated: true,
      note: coverage?.note || 'Есть в общей выручке KPI iiko, но не распределено по dish_sales.'
    });
  }

  return list.sort((a, b) => toNumber(b.revenue) - toNumber(a.revenue));
}
...
function buildDiscountAnalytics(rows, startDate, endDate, revenueBase = 0) {
  ...
  }).filter((item) => item.revenue || item.discounts).sort((a, b) => b.discounts - a.discounts);
  ...
  const worstDay = [...days].filter((item) => item.discounts > 0).sort((a, b) => b.percent - a.percent || b.discounts - a.discounts)[0] || null;
  const riskyDays = [...days].filter((item) => item.percent > 5).sort((a, b) => b.percent - a.percent || b.discounts - a.discounts).slice(0, 5);
  const topDaysByAmount = [...days].filter((item) => item.discounts > 0).sort((a, b) => b.discounts - a.discounts).slice(0, 5);

  const insight = totalDiscounts
    ? `Скидки ${formatMoney(totalDiscounts)} — ${totalPercent}% от продаж. ${worstChannel ? `Больше всего скидок в канале ${worstChannel.name}: ${worstChannel.discountsText}.` : ''} ${worstDay ? `Самый высокий процент по дням: ${worstDay.label}, ${worstDay.percentText}.` : ''}`.trim()
    : 'Скидок за выбранный период нет.';

  const advice = status === 'good'
    ? 'Скидки выглядят нормально. Продолжать контроль процента, а не только суммы в рублях.'
    : status === 'warn'
      ? 'Скидки выше комфортного уровня. Проверить дни и каналы с максимальным процентом.'
      : 'Скидки в зоне риска. Нужна разборка причин, каналов и ручных скидок.';
...
function aggregateHourlyRows(rows, revenueBase = 0) {
  ...
  return [...grouped.entries()].map(([hour, list]) => {
...
function buildHourlyAnalytics(rows, revenueBase = 0) {
  ...
  const lunchShare = revenueBase ? Math.round((lunchRevenue / revenueBase) * 100) : 0;
  const eveningShare = revenueBase ? Math.round((eveningRevenue / revenueBase) * 100) : 0;
...
function cardRangeFromInput(input) {
  if (input && typeof input === 'object') {
    return { startDate: input.startDate, endDate: input.endDate };
  }
  const date = String(input || getDefaultOperationalDate()).slice(0, 10);
  return { startDate: date, endDate: date };
}

function buildRestaurantCards(restaurants, dailyRows, kpis, rangeOrDate, kpiRows = []) {
  const cardRange = cardRangeFromInput(rangeOrDate);
  const daysCount = daysBetweenInclusive(cardRange.startDate, cardRange.endDate);

  return restaurants.map((restaurant) => {
    const normalizedRestaurantId = normalizeKpiRestaurantId(restaurant.id);
    const kpiSalesRows = (Array.isArray(kpiRows) ? kpiRows : []).filter((item) => normalizeKpiRestaurantId(item.restaurant_id) === normalizedRestaurantId && inDateRange(item, cardRange.startDate, cardRange.endDate));
    const dailyPeriodRows = (Array.isArray(dailyRows) ? dailyRows : []).filter((item) => item.restaurant_id === restaurant.id && inDateRange(item, cardRange.startDate, cardRange.endDate));
    const rows = kpiSalesRows.length ? kpiSalesRows : dailyPeriodRows;
    ...
    const avgCheck = checks ? Math.round(revenue / checks) : Math.round(average(rows, 'avg_check'));
    const planFromRows = sum(dailyPeriodRows, 'plan_revenue');
    const plan = planFromRows || (toNumber(kpi.daily_revenue_plan) || DEFAULT_DAILY_PLAN) * daysCount;
    ...
    return {
      ...
      problem,
      status,
      dataQuality: kpiSalesRows.length ? 'kpi_sales_ready_by_period' : 'revenue_ready_checks_guests_need_calibration',
      note: kpiSalesRows.length
...
function buildZeroSummary({ restaurantId, restaurants, selectedRestaurants, range, allDailyRows, allKpis }) {
  ...
  return {
    ...
    moneyLosses: [{ title, amount: 0, reason: 'КЛИК ждёт первые чеки из iiko.', action: 'После первого чека n8n запишет данные, и экран обновится автоматически.', level: 'neutral' }],
    totalLoss: 0,
    actionPlan: [{ role: 'КЛИК', title, text: 'Пока можно проверить готовность смены, цели по среднему чеку и план на день.' }],
    teamScript: 'Продаж пока нет. После первого чека КЛИК сформирует скрипт для смены на основе фактических данных.',
...
export async function getSupabaseSummary({ restaurantId = 'all', date, period = 'day' } = {}) {
  ...
  const rawCategories = aggregateCategories(periodDishes);
  const menuCoverage = buildMenuCoverage(rawCategories, revenue);
  const categories = categoriesWithUnallocated(rawCategories, menuCoverage).slice(0, 10);
  ...
  return {
    dataMode: 'supabase_klik_v8_discount_analytics_ready',
    isEmptyPeriod: false,
    generatedAt: new Date().toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    selectedRestaurantId: activeRestaurant.id,
    ...
    network: { selectedRestaurantId: activeRestaurant.id, restaurants: allRestaurantCards, totals: { revenue: networkRevenue, plan: networkPlan, percent: networkPlan ? Math.round((networkRevenue / networkPlan) * 100) : 0, avgCheck: networkAvgCheck, checks: networkChecks, weakPoints: allRestaurantCards.filter((item) => item.status !== 'good').length }, ai: `КЛИК анализирует период ${range.startDate} — ${range.endDate}.` },
    moments: [
      ...
      hourlyAnalytics.bestHour ? { title: 'Пик продаж по часам', text: `${hourlyAnalytics.bestHour.label}: ${hourlyAnalytics.bestHour.revenueText}`, level: 'good' } : null,
    ...
    moneyLosses: [
      ...
      { title: discountLoss > 0 ? 'Скидки требуют контроля' : 'Скидки в норме', amount: discountLoss, reason: `${formatMoney(discounts)} скидок, около ${discountPercent}% от продаж`, action: discountLoss > 0 ? (discountAnalytics.worstChannel ? `Проверить канал ${discountAnalytics.worstChannel.name} и день ${discountAnalytics.worstDay?.label || 'с максимальными скидками'}.` : 'Проверить дни и каналы с максимальными скидками.') : 'Продолжать отслеживать процент скидок.', level: discountLoss > 0 ? 'warn' : 'good' }
    ],
    totalLoss,
    actionPlan: [
    ...
    forecast: { current: revenue, plan: planRevenue, projected, risk: projected < planRevenue ? 'Риск не выполнить план' : 'План можно выполнить', gap: planRevenue - projected, confidence: week.length >= 7 ? 78 : 55, recommendations: forecastRecommendations },
    kpiSettings: [
      { name: 'План дня', value: formatMoney(DEFAULT_DAILY_PLAN), status: 'редактируется в Управлении' },
      { name: 'План недели', value: formatMoney(DEFAULT_WEEKLY_PLAN), status: 'редактируется в Управлении' },
      { name: 'План месяца', value: formatMoney(DEFAULT_MONTHLY_PLAN), status: 'редактируется в Управлении' },
    ...
    dataQuality: {
      ...
      menu: menuCoverage.status === 'good'
        ? `готово: категории покрывают ${menuCoverage.coveragePercent}% выручки периода`
        : `частично: категории покрывают ${menuCoverage.coveragePercent}% выручки периода, остаток показан как Не распределено`,

...
function buildRestaurantCards(restaurants, dailyRows, kpis, currentDate) {
  return restaurants.map((restaurant) => {
    ...
    // For separate restaurants we currently trust revenue by point.
    // Checks/guests/avgCheck in daily_sales are not fully calibrated yet, so do not mark a point as bad because of avgCheck.
    ...
    return {
      ...
      problem,
      status,
      dataQuality: 'revenue_ready_checks_guests_need_calibration',
      note: 'По точке выручка корректна; чеки, гости и средний чек требуют отдельной калибровки.'
    };
  });
}
...
export async function getSupabaseSummary({ restaurantId = 'all', date, period = 'day' } = {}) {
  ...
  const shouldUseKpiSales = restaurantId === 'all';
  const periodKpiSales = shouldUseKpiSales ? kpiSalesRows.filter((row) => inDateRange(row, range.startDate, range.endDate)) : [];
  const previousKpiSales = shouldUseKpiSales ? kpiSalesRows.filter((row) => inDateRange(row, previousStart, previousEnd)) : [];
  ...
  const trendRows = shouldUseKpiSales ? kpiSalesRows : dailyRows;
  ...
  return {
    ...
    dataQuality: {
      ...
      restaurants: 'частично: выручка по Азиаток/Акватория сходится, чеки и гости по точкам требуют калибровки',

...
export async function getSupabaseSummary({ restaurantId = 'all', date, period = 'day' } = {}) {
  ...
  const hourlyRowsForSelection = selectedKpiRestaurantId === 'all'
    ? hourlyRows.filter((row) => normalizeKpiRestaurantId(row.restaurant_id) === 'all')
    : hourlyRows.filter((row) => normalizeKpiRestaurantId(row.restaurant_id) === selectedKpiRestaurantId);

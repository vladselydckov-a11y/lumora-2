...
export async function getSupabaseSummary({ restaurantId = 'all', date, period = 'day' } = {}) {
  ...
  const [allKpis, allDailyRows, dishRows, waiterRows, channelRows, kpiSalesRows] = await Promise.all([
  ...
  const hasAnyPeriodData = periodKpiSales.length || periodRows.length || periodDishes.length || periodWaiters.length || periodChannels.length;
  if (!hasAnyPeriodData) {
  ...
  const discountPercent = revenue + discounts ? Number(((discounts * 100) / (revenue + discounts)).toFixed(1)) : 0;
  const discountPercentStatus = discountPercent <= 5 ? 'good' : discountPercent <= 8 ? 'warn' : 'bad';
  ...
  const forecastRecommendations = [
    ...
    discounts ? 'Разобрать скидки по дням и каналам, особенно если процент выше обычного.' : 'Продолжать отслеживать скидки.',
  ...
  const aiRecommendations = [
    ...
    'Разобрать скидки по дням и каналам.',
  ...
  return {
    dataMode: 'supabase_lumora_v8_client_safe_final',
    isEmptyPeriod: false,
    generatedAt: new Date().toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    selectedRestaurantId: activeRestaurant.id,
    ...
    moneyLosses: [
      ...
      { title: discountLoss > 0 ? 'Скидки требуют контроля' : 'Скидки в норме', amount: discountLoss, reason: `${formatMoney(discounts)} скидок, около ${discountPercent}% от продаж`, action: discountLoss > 0 ? 'Проверить дни и каналы с максимальными скидками.' : 'Продолжать отслеживать процент скидок.', level: discountLoss > 0 ? 'warn' : 'good' }
    ],
    totalLoss,
    actionPlan: [
    ...
    dataQuality: {
      ...
      hourly: 'готово: выручка по часам сходится с KPI',
      discounts: 'готово: скидки считаются, статус лучше оценивать по проценту от продаж',

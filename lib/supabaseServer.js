...
export async function getSupabaseSummary({ restaurantId = 'all', date, period = 'day' } = {}) {
  ...
  const elapsedDays = daysBetweenInclusive(range.startDate, range.endDate);
  const monthStartDate = monthStart(range.endDate);
  const monthStartObj = dateToUTC(monthStartDate);
  const nextMonthObj = new Date(monthStartObj);
  nextMonthObj.setUTCMonth(nextMonthObj.getUTCMonth() + 1);
  const nextMonthStart = nextMonthObj.toISOString().slice(0, 10);
  const monthEndDate = dateMinusDays(nextMonthStart, 1);
  const daysInMonth = daysBetweenInclusive(monthStartDate, monthEndDate);
  const dailyTempo = elapsedDays ? revenue / elapsedDays : 0;

  const projected =
    range.period === 'month'
      ? Math.round(dailyTempo * daysInMonth)
      : range.period === 'week'
        ? Math.round(dailyTempo * 7)
        : Math.round(revenue * 1.12);

  const projectedMonth = Math.round(dailyTempo * daysInMonth);
  ...
  return {
    ...
    forecast: {
      ...
      projectedMonth,
      monthPlan: DEFAULT_MONTHLY_PLAN,
      monthlyTempo: dailyTempo,
      risk: projected < planRevenue ? 'Риск не выполнить план' : 'План можно выполнить',
      monthRisk: projectedMonth < DEFAULT_MONTHLY_PLAN ? 'При текущем темпе месяц ниже плана' : 'При текущем темпе месяц может выполнить план',
      gap: planRevenue - projected,
      monthGap: DEFAULT_MONTHLY_PLAN - projectedMonth,

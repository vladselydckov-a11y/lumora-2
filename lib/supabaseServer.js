...
export async function getSupabaseSummary({ restaurantId = 'all', date, period = 'day' } = {}) {
  ...
  const aiRecommendations = [
    ...
    'Использовать рейтинг блюд для продвижения сильных позиций.',
  ...
  return {
    dataMode: 'supabase_lumora_v8_kpi_sales_periods',
    isEmptyPeriod: false,
    generatedAt: new Date().toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    selectedRestaurantId: activeRestaurant.id,
    ...
    actionPlan: [
      ...
      { role: 'Меню', title: topDishes[0] ? `Продвигать ${topDishes[0].name}` : 'Проверить меню', text: topDishes[0] ? `Лидер периода: ${topDishes[0].revenue}. Использовать в рекомендациях и акциях без потери маржи.` : 'Нет данных по блюдам за период.' }
    ],

...
function getRowRestaurantText(row) {
  return String(
    row?.restaurant_id ||
    row?.restaurantId ||
    row?.restaurant ||
    row?.restaurant_name ||
    row?.restaurantName ||
    row?.restaurant_section ||
    row?.restaurantSection ||
    row?.RestaurantSection ||
    row?.section ||
    row?.['РесторанРаздел'] ||
    row?.['Раздел'] ||
...
function normalizeRowRestaurantId(row) {
  const direct = normalizeKpiRestaurantId(row?.restaurant_id || row?.restaurantId);
  if (['all', 'aziatok', 'akvatoria'].includes(direct)) return direct;

  const text = getRowRestaurantText(row);
  if (text.includes('акват') || text.includes('akvator')) return 'akvatoria';
  if (text.includes('азиат') || text.includes('aziatok') || text.includes('бар 96')) return 'aziatok';
  ...
  if (orderTypeText.includes('курьер')) return 'akvatoria';
  if (orderTypeText.includes('самовывоз') || orderTypeText.includes('pickup') || orderTypeText.includes('зал') || orderTypeText.includes('hall')) return 'aziatok';

  return '';
}

function filterRowsForRestaurant(rows, selectedRestaurantId) {
  const list = Array.isArray(rows) ? rows : [];
  const selectedId = normalizeKpiRestaurantId(selectedRestaurantId);

  if (selectedId === 'all') {
    const exactAll = list.filter((row) => normalizeRowRestaurantId(row) === 'all');
    return exactAll.length ? exactAll : list.filter((row) => normalizeRowRestaurantId(row) !== 'all');
  }

  const exactRows = list.filter((row) => normalizeRowRestaurantId(row) === selectedId);
  if (exactRows.length) return exactRows;

  return [];
}

function calculateFoodcostFromDishRows(rows, fallbackRows = []) {
  const visibleRows = cleanDishRows(rows);
  const rowsWithCost = visibleRows.filter((row) => toNumber(row?.cost) > 0 && toNumber(row?.revenue) > 0);
  const dishRevenue = sum(rowsWithCost, 'revenue');
  const dishCost = sum(rowsWithCost, 'cost');

  if (dishRevenue && dishCost) {
    return {
      available: true,
      value: Number(((dishCost / dishRevenue) * 100).toFixed(1)),
      cost: dishCost,
      revenue: dishRevenue,
      source: 'dish_sales_cost'
    };
  }

  const fallbackWithFoodcost = (Array.isArray(fallbackRows) ? fallbackRows : []).filter((row) => toNumber(row?.foodcost_percent) > 0);
  if (fallbackWithFoodcost.length) {
    return {
      available: true,
      value: Number(weightedPercent(fallbackWithFoodcost).toFixed(1)),
      cost: 0,
      revenue: sum(fallbackWithFoodcost, 'revenue'),
      source: 'daily_sales_foodcost_percent'
    };
  }

  return { available: false, value: 0, cost: 0, revenue: 0, source: 'not_connected' };
}

function buildRestaurantCards(restaurants, dailyRows, kpis, currentDate, kpiRows = []) {
  return restaurants.map((restaurant) => {
    const normalizedRestaurantId = normalizeKpiRestaurantId(restaurant.id);
    const kpiSalesRow = kpiRows.find((item) => normalizeKpiRestaurantId(item.restaurant_id) === normalizedRestaurantId && item.business_date === currentDate) || null;
    const dailyRow = dailyRows.find((item) => item.restaurant_id === restaurant.id && item.business_date === currentDate) || {};
    const row = kpiSalesRow || dailyRow;
    ...
    const plan = toNumber(dailyRow.plan_revenue) || toNumber(kpi.daily_revenue_plan) || DEFAULT_DAILY_PLAN;
    ...
    return {
      ...
      problem,
      status,
      dataQuality: kpiSalesRow ? 'kpi_sales_ready_by_restaurant_section' : 'revenue_ready_checks_guests_need_calibration',
      note: kpiSalesRow
        ? 'По точке выручка, чеки, гости и средний чек идут из kpi_sales через RestaurantSection.'
...
export async function getSupabaseSummary({ restaurantId = 'all', date, period = 'day' } = {}) {
  ...
  const channelRowsForSelection = filterRowsForRestaurant(channelRows, selectedKpiRestaurantId);
  ...
  const hourlyRowsForSelection = restaurantId === 'all'
    ? hourlyRows.filter((row) => normalizeRowRestaurantId(row) !== 'all')
    : filterRowsForRestaurant(hourlyRows, selectedKpiRestaurantId);
  ...
  const foodcostInfo = calculateFoodcostFromDishRows(periodDishes, periodRows);
  const foodcostAvailable = foodcostInfo.available;
  const foodcost = foodcostInfo.value;
  ...
  const allRestaurantCards = buildRestaurantCards(restaurants, allDailyRows, allKpis, range.endDate, kpiSalesRows);
  ...
  return {
    ...
    dataQuality: {
      ...
      channels: restaurantId === 'all'
        ? 'готово: каналы считаются по всей сети'
        : 'готово: каналы и скидки фильтруются по выбранной точке',
